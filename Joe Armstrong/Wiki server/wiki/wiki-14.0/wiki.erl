...
%% Comment out the next line to restrict the hosts which query the
%% wiki to a particular SICS domain
 
event_handler(Request, State) -> event_handler1(Request, State+1);
event_handler(Request={Op, From, Url, Args}, State) ->
    %% io:format("From:~p ~p~n",[From, Url]),
    case From of
	{ok, {193,10,N,_},_} when N == 64; N==65;N==66;N==67;N==135  ->
	    event_handler1(Request, State + 1);
	{ok, Ip, Name} ->
	    {show({notInSICS,Ip,Name}), State +1};
	nomatch ->
	    {show(notInSICS), State + 1}
    end.

event_handler1({_,From,"/",[]}, State) ->
    event_handler1({a,From,"/wiki/showPage",[{"node","home"}]}, State); 
event_handler1({_,From,"/wiki/",[]}, State) ->
    event_handler1({post,From,"/wiki/showPage",[{"node","home"}]}, State); 
event_handler1({_,From,"/wiki/showPage",Args}, State) ->
    {showPage(Args), State+1};
event_handler1({_,From,"/wiki/showOldPage",Args}, State) ->
    {showOldPage(Args), State+1};
event_handler1({_,From,"/wiki/editPage",Args}, State) ->
    {editPage(Args, From), State+1};
event_handler1({_,From,"/wiki/editTag",Args}, State) ->
    {editTag(Args, From), State+1};
event_handler1({_,From,"/wiki/storePage",Args}, State) ->
    {storePage(Args, From), State+1};
event_handler1({_,From,"/wiki/storeTagged",Args}, State) ->
    {storeTagged(Args, From), State+1};
event_handler1({_,From,"/wiki/storeNewPage",Args}, State) ->
    {storeNewPage(Args, From), State+1};
event_handler1({_,From,"/wiki/showHistory",Args}, State) ->
    {showHistory(Args, From), State+1};
event_handler1({_,From,"/wiki/previewPage",Args}, State) ->
    {previewPage(Args, From), State+1};
event_handler1({_,From,"/wiki/previewTagged",Args}, State) ->
    {previewTagged(Args, From), State+1};
event_handler1({_,From,"/wiki/previewNewPage",Args}, State) ->
    {previewNewPage(Args, From), State+1};
event_handler1({_,From,"/wiki/createNewPage",Args}, State) ->
    {createNewPage(Args, From), State+1};
event_handler1({_,From,"/wiki/sendMeThePassword",Args}, State) ->
    {sendMeThePassword(Args, From), State+1};
event_handler1({_,From,"/wiki/allPages",Args}, State) ->
    {allPages(), State+1};
event_handler1({_,From,"/wiki/lastEdited",Args}, State) ->
    {lastEdited(), State+1};
event_handler1({_,From,"/wiki/wikiZombies",Args}, State) ->
    {wikiZombies(), State+1};
event_handler1({_,From,"/wiki/allRefsToMe",Args}, State) ->
    {allRefsToMe(Args), State+1};
event_handler1({_,_,"/wiki/image/" ++ File,Args}, State) ->
